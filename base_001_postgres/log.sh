...
# version: 1.0.0
# description: json logger that sends log statements to stout as json

if [ -n "${BASHLOG_JSON_PATH+set}" ]; then
...
function publish_json_logs_info() {
    while read line; do
...
function publish_json_logs_error() {
    while read line; do
        ESCAPED_LINE=$(echo ${line} | sed 's/"/\\"/g')
        log error $ESCAPED_LINE;
    done <${1}
}

function publish_json_logs() {
    publish_json_logs_info $ETL_LOG_STDOUT;
    publish_json_logs_error $ETL_LOG_STDERR;
}
...
function log() {
  ...
  local json_stout="${BASHLOG_JSON_STOUT:-0}";
