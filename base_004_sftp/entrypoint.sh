...
if [ ! -f "$userConfFinalPath" ]; then
    ...
    if [ -f "$userConfFinalPath" ] && [ "$(wc -l < "$userConfFinalPath")" -gt 0 ]; then
        ...
        log error "FATAL: No users provided!"
        exit 3
    fi

    # Generate unique ssh keys for this container, if needed
    if [ ! -f /etc/ssh/ssh_host_ed25519_key ]; then
        log info $(ssh-keygen -t ed25519 -f /etc/ssh/ssh_host_ed25519_key -N '')
    fi
    if [ ! -f /etc/ssh/ssh_host_rsa_key ]; then
        log info $(ssh-keygen -t rsa -b 4096 -f /etc/ssh/ssh_host_rsa_key -N '')
    fi
...
if [ -d /etc/sftp.d ]; then
    for f in /etc/sftp.d/*; do
        if [ -x "$f" ]; then
            ...
            log error "Could not run $f, because it's missing execute permission (+x)."
        fi
    done
    unset f
fi
